...
static void
replace_fval_in_params(int entry)
{
  int fval, fvalsame, newdsc, newdscsame, newarg, newargsame;
  int params, narg, i;

  fval = FVALG(entry);
  fvalsame = LOWER_SYMBOL_REPLACE(fval);
  newdsc = NEWDSCG(fval);
  newarg = NEWARGG(fval);
...
static void
replace_sptr_in_ast(int sptr)
{
  int newsptr, ast;

  if (sptr == 0 || sptr == NOSYM) {
    return;
  }
  newsptr = LOWER_SYMBOL_REPLACE(sptr);
  if (newsptr == 0 || newsptr == NOSYM){
    return;
  }
  ast = mk_id(sptr);
  A_SPTRP(ast, newsptr);
...
static inline void
add_replace_map(int sptr, int newsptr)
{
  if (sptr == 0 || sptr == NOSYM || newsptr == 0 && newsptr == NOSYM) {
    return;
  }
...
static void
replace_fval_in_ast(int fval)
{
  int fvalsame, var, var_same;
    ...
    for (ent = gbl.entries; ent > NOSYM; ent = SYMLKG(ent)) {
      for (esame = gbl.entries; esame != ent; esame = SYMLKG(esame)) {
        ...
        if (fval && fvalsame && fval != fvalsame &&
          ...
          replace_fval_in_params(ent);
          FVALP(ent, fvalsame);
          replace_fval_in_ast(fval);
